...
# =============================
    ...
    with st.sidebar:
        ...
        run_s = st.button("Calcular índices", type="primary")
    st.subheader("🍂 Índices de estacionalidad por mes del año")
    if run_s:
        # Agregados mensuales por mes natural (1..12) a lo largo de todos los años
        dfm = raw.dropna(subset=['Fecha entrada','Fecha salida']).copy()
        # Construir noches e ingresos por mes de llegada (aprox por reserva, sin prorrateo)
        dfm['los'] = (dfm['Fecha salida'].dt.normalize() - dfm['Fecha entrada'].dt.normalize()).dt.days.clip(lower=1)
        dfm['Mes'] = dfm['Fecha entrada'].dt.month
        bym = dfm.groupby('Mes').agg(noches=('los','sum'), ingresos=('Precio','sum')).reset_index()
        bym['ADR'] = np.where(bym['noches']>0, bym['ingresos']/bym['noches'], np.nan)
        if met=="Noches":
            v = bym['noches']
        elif met=="Ingresos (€)":
            v = bym['ingresos']
            ...
            v = bym['ADR']
        idx = v / (v.mean() if v.mean()!=0 else 1)
        out = pd.DataFrame({'Mes': bym['Mes'], 'Índice': idx})
        out['Mes'] = out['Mes'].map({1:'Ene',2:'Feb',3:'Mar',4:'Abr',5:'May',6:'Jun',7:'Jul',8:'Ago',9:'Sep',10:'Oct',11:'Nov',12:'Dic'})
        st.line_chart(out.set_index('Mes'), height=280)
        st.dataframe(bym, use_container_width=True)
